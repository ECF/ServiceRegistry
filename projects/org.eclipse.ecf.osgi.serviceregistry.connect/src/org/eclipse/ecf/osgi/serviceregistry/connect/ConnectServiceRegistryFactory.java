	...
	public ServiceRegistry newServiceRegistry(
		...
		try {
			...
			if (configuration == null) {
				// If configuration is null, we still setup bundles from classpath
				pojoSRConfig.put(PojoServiceRegistryFactory.BUNDLE_DESCRIPTORS, convertBundleDescriptors(new ClasspathBundleFinder().findBundles()));
			} else {
				...
				for(String ck: configuration.keySet()) {
					if (ck.startsWith(BUNDLE_FINDERS)) {
						...
						Collection<BundleFinder> bundleFinders = (Collection<BundleFinder>) configuration.get(ck);
						if (bundleFinders != null)
							for(BundleFinder finder: bundleFinders) 
								pojoSRDescriptors.addAll(convertBundleDescriptors(finder.findBundles()));
					}
					if (ck.startsWith(BUNDLE_DESCRIPTORS)) {
						@SuppressWarnings("unchecked")
						Collection<BundleDescriptor> bundleDescriptors = (Collection<BundleDescriptor>) configuration.get(ck);
						if (bundleDescriptors != null)
							pojoSRDescriptors.addAll(convertBundleDescriptors(bundleDescriptors));
					}
				}
				pojoSRConfig.put(PojoServiceRegistryFactory.BUNDLE_DESCRIPTORS, pojoSRDescriptors);
			}
